...
- HoST v2 learns 23-DoF G1 standing-up without reference trajectories through stage-aware reward groups, four critics, a removable vertical-force curriculum, decreasing action bounds and L2C2/action smoothness. Table III identifies the training components and Table IV reports 20/20 versus 11/20 without L2C2. Official MIT code at `70bb580949a336a920833700e4b5dc3bf7fe87ce` maps the force/action curricula and multi-critic advantages.
- FRASA v3 reduces Sigmaban recovery to five symmetric sagittal DoFs and uses randomized fallen states, CrossQ+SAC and actuator/delay-aware MuJoCo transfer. Tables III/IV report repeated real stand-up and pendulum comparisons, while the paper acknowledges manually rejecting high-stress policies and weak lateral versatility. Official MIT code at `78457df8fb1533b9bbda60a345c015c87cdf9732` maps observations/actions, termination, dynamics randomization and initial falls.
- The frozen corpus is complete at 14 papers: exactly two original interpretations and two bounded Engineering Claims in each of seven domains, backed by 28 normalized paper/code-or-status sources. The offline checker and full acceptance suite are now release gates rather than a manually maintained completion assertion.
