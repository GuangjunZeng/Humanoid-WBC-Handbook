...
EXPECTED_BASELINE_PROBLEMS = 203
...
class WebSearchTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.problems = collect_web_problems(cls.sources, cls.claims)
    ...
    def test_future_draft_claim_is_searchable_at_l1(self):
        ...
        self.assertEqual(page.search_item()["id"], draft.claim_id)
    ...
    def test_search_items_do_not_leak_credibility_fields(self):
        for problem in self.problems:
            ...
            visible_card_text = f"{item['title']} {item['summary']}".casefold()
            indexed_text = " ".join(
                " ".join(value) if isinstance(value, list) else str(value)
                for key, value in item.items()
                if key != "url"
            ).casefold()
            for status in ("resolved", "partial", "unresolved", "conflicting"):
                status_token = rf"\b{status}\b"
                self.assertNotRegex(visible_card_text, status_token)
                self.assertNotRegex(indexed_text, status_token)
    ...
    def test_render_and_index_are_deterministic_and_checked(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(first["written"], len(self.problems))
