...
- Recovery sequence: install and enable the ChatGPT extension in the same signed-in work Chrome profile, reinstall/repair the Browser plugin from the Codex plugin UI to restore its native messaging bridge, then rerun the Chrome connection check before resuming the Xiaohongshu acceptance test.
