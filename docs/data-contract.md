...
### English problem localization

Chinese evidence records remain canonical and Chinese-first. Each searchable
problem has one reviewed presentation-layer translation at
`data/locales/en/problems/<stable-id>.json`. The record contains
`schema_version`, `problem_id`, `kind`, `source_fingerprint`, and an exact
`translations` map for every visible narrative segment required by its Claim or
QA page. It never duplicates URLs, confidence numbers, status enums, versions,
or evidence relations; those are derived from canonical data.

The fingerprint is computed from the stable problem ID, problem kind, semantic
segment keys, and their current Chinese source text. Missing, extra, or stale
records block both problem rendering and web-index generation. English terms are
English-first with a Chinese counterpart; code identifiers, mathematical symbols,
CLI flags, product names, versions, and original evidence URLs retain their exact
spelling. Generated Chinese pages stay at `content/problems/<id>.md`; English
pages use `content/problems/en/<id>.md` and the two pages link to each other.

The schema-v2 web index keeps one item per problem. Its `title`, `summary`, and
`url` objects contain `zh` and `en` values, while aliases and search text combine
both languages. Credibility and review fields remain forbidden on the search
surface and appear only on detail pages.
