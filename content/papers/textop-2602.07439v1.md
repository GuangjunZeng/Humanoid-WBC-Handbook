...
TextOp 每次只生成一小段未来，同时保留最近历史以衔接新口令。历史帧提供当前运动相位，短块则限制一次文字更新影响的时间范围。
...
第三个洞察是“生成增强”不会在所有分布上单调变好。Table IV/V 中混合数据改善训练生成器分布上的表现，但在未见 SnapMoGen 上仅 MoCap 反而最好，说明该增强是分布适配而不是普遍泛化增益。
...
![Figure 2-3：流式文字生成、数据对齐与低层跟踪](assets/textop-2602.07439v1/figure-2-3-streaming.jpg)
