...
可以把 ASE 理解成先学一套没有标签的“动作词典”：判别器保证每个词像人类动作，encoder 迫使不同 latent 对应不同词，高层任务只需组合词而不用重新学习每块肌肉。另一个类比是地图坐标：把 latent 约束在球面避免走到无限远的未知区域，但高层先在未归一化空间采样，再投影到球面，让探索仍能从中心向各种方向展开。
...
面向机器人，建议在 latent 上再加一层 command semantics：只开放“站立、低速前进、转向、低能量恢复”等经过验证的区域，高层请求超出时投影或拒绝。这个安全层好比在一张连续地图上只开放通过勘测的道路，而不是因为整个地图画出来就允许车辆到任意坐标。
