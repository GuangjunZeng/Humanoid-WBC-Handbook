...
动捕数据规模大，却通常没有施加在仿真人体上的 action。仅做 behavior cloning 不知道哪些控制产生动作；仅用在线 RL 又难以覆盖人类动作分布。问题像拿到大量精彩比赛录像，却没有运动员的肌肉指令：模型既要从录像学习“哪些状态像人”，又要靠交互学会“怎样控制到那里”。
...
仅有 FB 可能学会到达状态，却不保证动作像人。CPR 增加 latent-conditioned discriminator，区分 motion dataset 与策略占用，并估计密度比；critic 与 actor 同时结合 FB 回报和 discriminator 回报。它像给能到目的地的导航器再加一位动作教练：目标仍由任务决定，路径还要留在人类动作分布附近。
...
从模拟人体迁移到机器人时，可以把高层表示当作检索或生成候选，再交给有约束的重定向和低层控制器。这个两层结构好比先由编舞提出动作，再由机械工程师检查舞台与演员能力；直接把模拟人体动作映射到电机，会把形态和执行器差异全部压给一个未经验证的缩放。
