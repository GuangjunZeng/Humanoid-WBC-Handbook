...
可以把动作处理理解成先检查乐谱是否能由这件乐器演奏：超出音域的段落先剔除，节拍和落点先校正，再让演奏者练习。自适应 σ 像教练逐步收紧容差：初学时允许拳路偏差，动作能完成后再要求手落在更小靶心；若第一天就用比赛标准，学员可能完全得不到有效反馈。
...
每动作一个 policy 使部署选择成为安全关键。动作名称不能直接映射 checkpoint，必须验证机器人型号、reference hash、PD、observation schema、起始姿态和可用空间。选择错 policy 好比把不同选手的编舞和音乐错配，即使两者单独正确也会立即失步。策略 manifest 应自动拒绝不匹配。
