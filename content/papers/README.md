...
本目录分成三层：[`catalog.json`](catalog.json) 是经典论文、官方开源工作和板块缺口的覆盖目录；[`registry.json`](registry.json) 只包含已通过全文解读质量门的论文；[`project-catalog.json`](project-catalog.json) 单独记录高质量开源项目。当前论文快照收录 76 篇唯一作品，其中 36 篇已有完整中英文深度解读、40 篇经主来源核验后待深读；项目快照收录 43 个仓库，其中 24 个已有固定 commit 的完整中英文代码解读页。
...
| [动作生成与可命令行为](domains/motion-generation.md) | 20 | 10 | 10 | 20 |
