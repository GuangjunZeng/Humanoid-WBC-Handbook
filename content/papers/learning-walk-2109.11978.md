...
GPU 物理仿真提供了另一种工作方式：不是让一台机器人经历更长时间，而是让几千台机器人同时经历不同地形和扰动。它像一次让整座驾校同时开课，教练根据每辆车的进展调整赛道，而不是排队等一名学员跑完。问题随之变成 batch、horizon、课程和显存之间的系统配比。
...
第三，时间截断自举纠正固定 episode 长度带来的假终止。可以把 time-out 理解为比赛录像被剪断，而不是机器人真正摔倒；价值目标应从剪断处的下一状态继续估计。实验表明这一处理可带来约 10–20% 回报提升，且短于约 25 步的 horizon 明显伤害 PPO。
