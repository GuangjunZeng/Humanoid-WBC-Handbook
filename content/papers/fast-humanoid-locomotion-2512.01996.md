...
人形强化学习（reinforcement learning, RL）通常使用近端策略优化（Proximal Policy Optimization, PPO），因为大规模并行仿真能快速生成新数据，同策略（on-policy）实现相对稳定。代价是同一状态转移（transition）只使用很少次数，墙钟速度受采样轨迹（rollout）与更新平衡限制。把 SAC/TD3 直接移植到高维人形机器人又常因回放分布、评论家网络（critic）过估计、探索过强和大批量优化不稳而失败。

离策略经验回放会重复利用历史状态转移，但也引入回放分布陈旧与评论家偏差风险；论文的速度来自一组互相依赖的评论家、优化器（optimizer）、探索噪声、奖励和仿真器配方，不是某个单独超参数。
