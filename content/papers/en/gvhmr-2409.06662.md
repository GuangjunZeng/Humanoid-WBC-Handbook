...
The Gravity-View frame is defined by world gravity and the camera viewing direction for each image. The network predicts human orientation relative to this per-frame frame, then uses relative camera rotation to bring those orientations into one gravity-aligned world frame. This anchors pitch and roll receive a repeated physical anchor, while yaw still has to be connected over time.
...
Post-processing detects joints expected to remain stationary over a temporal interval and applies IK to stabilize them in the world. This constrains predicted stationary joints in world coordinates. It can reduce visible sliding, but a wrong stationary prediction can conceal a network failure by distorting the body. Raw and post-processed trajectories should both be retained.
